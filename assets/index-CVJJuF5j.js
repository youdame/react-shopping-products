...
const Spinner = ({ size: size2 = "medium" }) => {
  return /* @__PURE__ */ jsx$1("div", { css: containerStyle, children: /* @__PURE__ */ jsx$1(
    "div",
    {
      css: [
        baseSpinnerStyle,
        size2 === "small" && smallStyle,
        size2 === "medium" && mediumStyle,
        size2 === "large" && largeStyle
      ]
    }
  ) });
};
...
function createProductListViewModel({
  ...
  return products2.map((product) => {
    const matchedCart = cartItems2 == null ? void 0 : cartItems2.find((item) => item.product.id === product.id);
    return {
      id: product.id,
      title: product.name,
...
const handlers = [
  ...
  http.post("/cart-items", async ({ request }) => {
    ...
    const product = products.content.find((p2) => p2.id === productId);
    if (!product) {
      return HttpResponse.json(
        {
